...
const createUpdateNode = async (node, formData, salesforceBaseUrl, token) => {
  ...
  fieldMappings.forEach(mapping => {
    ...
    // Handle subfields (fields ending with _subfieldName)
    if (mapping.formFieldId.includes('_')) {
      const [baseFieldId, subfield] = mapping.formFieldId.split('_');
      
      // Check if this is a known subfield type (like fullname or address)
      if (baseFieldId in formData) {
        // For fullname subfields
        if (subfield === 'firstName' || subfield === 'lastName' || subfield === 'salutation') {
          value = formData[mapping.formFieldId] || '';
        }
        // For address subfields
        else if (subfield === 'street' || subfield === 'city' || subfield === 'state' || 
                 subfield === 'country' || subfield === 'postal') {
          value = formData[mapping.formFieldId] || '';
        }
        // For phone subfields
        else if (subfield === 'phoneNumber' || subfield === 'countryCode') {
          value = formData[mapping.formFieldId] || '';
        }
      }
    } 
    // Handle regular fields and picklist values
    else {
