...
// Helper function to upload files to Salesforce
async function uploadFilesToSalesforce(recordId, fileUploadFields, formData, salesforceBaseUrl, token) {
  if (!fileUploadFields || fileUploadFields.length === 0) {
    console.log('No file upload fields specified');
    return { success: true, uploadedFiles: [] };
  }

  const uploadedFiles = [];
  const errors = [];
  const contentVersionRequests = [];
  const fileMetadata = [];

  // Step 1: Collect all files and prepare ContentVersion requests
  for (const formFieldId of fileUploadFields) {
    const s3Urls = formData[formFieldId];

    if (!s3Urls) {
      console.warn(`No S3 URL found for form field: ${formFieldId}`);
      continue;
    }

    const urlsToProcess = Array.isArray(s3Urls) ? s3Urls : [s3Urls];

    for (const s3Url of urlsToProcess) {
      if (!s3Url) {
        console.warn(`Empty S3 URL in array for form field: ${formFieldId}`);
        continue;
      }
      ...
      try {
        ...
        if (!fileResponse.ok) {
          throw new Error(`Failed to download file from S3: ${fileResponse.status} ${fileResponse.statusText}`);
        }

        const arrayBuffer = await fileResponse.arrayBuffer();
        const base64String = Buffer.from(arrayBuffer).toString('base64');
        const fileName = extractFileNameFromS3Url(s3Url) || 'uploaded_file';
        ...
        // Prepare ContentVersion request
        contentVersionRequests.push({
          method: 'POST',
        ...
        console.error(`File processing error for field ${formFieldId}, URL ${s3Url}:`, error.message);
        errors.push({
          formFieldId,
          fileName: 'unknown',
          s3Url: s3Url,
  ...
  if (contentVersionRequests.length === 0) {
    return {
  ...
  // Step 2: Batch create all ContentVersion records
  try {
    console.log(`Creating ${contentVersionRequests.length} ContentVersion records in batch`);
    ...
    const batchResponse = await fetch(`${salesforceBaseUrl}/composite/batch`, {
      ...
      body: JSON.stringify({
        batchRequests: contentVersionRequests
      })
    });
    ...
    if (!batchResponse.ok || batchResult.hasErrors) {
      throw new Error(batchResult.message || 'Batch ContentVersion creation failed');
    }

    // Step 3: Extract ContentVersion IDs and prepare ContentDocumentLink requests
    const contentVersionIds = [];
    const contentDocumentLinkRequests = [];
    ...
    batchResult.results.forEach((result, index) => {
      if (result.statusCode >= 200 && result.statusCode < 300) {
        ...
        // Store metadata with ContentVersion ID
        fileMetadata[index].contentVersionId = contentVersionId;
      } else {
        errors.push({
          formFieldId: fileMetadata[index]?.formFieldId || 'unknown',
          fileName: fileMetadata[index]?.fileName || 'unknown',
          error: result.result?.message || `HTTP ${result.statusCode}`
        });
      }
    });

    if (contentVersionIds.length === 0) {
      throw new Error('No ContentVersion records were created successfully');
    }

    // Step 4: Query for ContentDocumentIds in batch
    console.log('Querying ContentDocumentIds for created ContentVersions');

    const queryRequests = contentVersionIds.map((id, index) => ({
      method: 'GET',
      url: `/services/data/v60.0/query/?q=SELECT+ContentDocumentId+FROM+ContentVersion+WHERE+Id='${id}'`
    }));

    const queryBatchResponse = await fetch(`${salesforceBaseUrl}/composite/batch`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
      ...
      body: JSON.stringify({
        batchRequests: queryRequests
      })
    });

    const queryBatchResult = await queryBatchResponse.json();

    if (!queryBatchResponse.ok || queryBatchResult.hasErrors) {
      throw new Error(queryBatchResult.message || 'Batch query failed');
    }

    // Step 5: Process query results and prepare ContentDocumentLink requests
    queryBatchResult.results.forEach((result, index) => {
      if (result.statusCode >= 200 && result.statusCode < 300) {
        const contentDocId = result.result.records[0]?.ContentDocumentId;
        if (contentDocId) {
          fileMetadata[index].contentDocumentId = contentDocId;

          // Prepare ContentDocumentLink request
          contentDocumentLinkRequests.push({
            method: 'POST',
          ...
          errors.push({
            formFieldId: fileMetadata[index]?.formFieldId || 'unknown',
            fileName: fileMetadata[index]?.fileName || 'unknown',
            error: 'No ContentDocumentId found'
          });
        }
      } else {
        errors.push({
    ...
    // Step 6: Batch create all ContentDocumentLink records
    if (contentDocumentLinkRequests.length > 0) {
      console.log(`Creating ${contentDocumentLinkRequests.length} ContentDocumentLink records in batch`);

      const linkBatchResponse = await fetch(`${salesforceBaseUrl}/composite/batch`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          batchRequests: contentDocumentLinkRequests
        })
      });

      const linkBatchResult = await linkBatchResponse.json();

      if (!linkBatchResponse.ok || linkBatchResult.hasErrors) {
        throw new Error(linkBatchResult.message || 'Batch ContentDocumentLink creation failed');
      }

      // Step 7: Process final results
      linkBatchResult.results.forEach((result, index) => {
        const metadata = fileMetadata[index];
        if (result.statusCode >= 200 && result.statusCode < 300) {
          uploadedFiles.push({
            formFieldId: metadata.formFieldId,
            fileName: metadata.fileName,
            contentDocumentId: metadata.contentDocumentId,
            success: true
          });
          console.log(`File uploaded and linked: ${metadata.fileName}`);
        } else {
          errors.push({
            formFieldId: metadata.formFieldId,
            fileName: metadata.fileName,
    ...
    console.error('Batch file upload error:', error.message);
    errors.push({
      formFieldId: 'batch',
      fileName: 'multiple',
      error: error.message
...
function extractFileNameFromS3Url(s3Url) {
  try {
    ...
    // Remove any URL encoding or query parameters from filename
    ...
    console.warn('Could not extract filename from S3 URL:', error);

    // Fallback: try to extract filename from the string directly
    const parts = s3Url.split('/');
    const lastPart = parts[parts.length - 1] || 'uploaded_file';
    return lastPart.split('?')[0].split('#')[0]; // Clean up
  }
}

// Updated createUpdateNode function
const createUpdateNode = async (userId, node, formData, salesforceBaseUrl, token) => {
  const { salesforceObject, conditions, fieldMappings, contentDocumentMappings = [], nodeId } = node;
  let matchedRecords = [];
  let tokenRefreshed = false;
  let newAccessToken;
  ...
  console.log('contentDocumentMappings==> ', JSON.stringify(contentDocumentMappings, null, 2));

  // Step 2: Extract file upload configuration (from contentDocumentMappings)
  let fileUploadFields = [];
  if (Array.isArray(contentDocumentMappings) && contentDocumentMappings.length > 0) {
    contentDocumentMappings.forEach(cfg => {
      if (Array.isArray(cfg.selectedFileUploadFields)) {
        fileUploadFields.push(...cfg.selectedFileUploadFields);
      }
    });
  }

  const actualFieldMappings = fieldMappings.filter(m => m.formFieldId !== undefined);

  console.log('File upload field IDs:', fileUploadFields);
  console.log('Actual field mappings:', JSON.stringify(actualFieldMappings, null, 2));
  console.log('Form data keys:', Object.keys(formData));

  // Step 3: Build payload from fieldMappings
  const payload = {};
  let hasValidMapping = false;

  actualFieldMappings.forEach(mapping => {
    const formFieldId = mapping.formFieldId || '';
    let value;

    if (formFieldId && typeof formFieldId === 'string' && formFieldId.includes('_')) {
      const [baseFieldId] = formFieldId.split('_');
      if (baseFieldId in formData) {
        value = formData[mapping.formFieldId] || '';
      }
    } else {
      value = formFieldId === '' && mapping.picklistValue !== undefined
        ? mapping.picklistValue
        : formData[formFieldId];
    }

    if (value !== undefined && value !== null) {
      if (mapping.fieldType === 'number' || mapping.fieldType === 'price') {
        value = Number(value);
      } else if (['picklist', 'shorttext', 'longtext'].includes(mapping.fieldType)) {
        value = String(value);
      }

      payload[mapping.salesforceField] = value;
      hasValidMapping = true;
      console.log(`Mapping ${formFieldId || 'predefined'} to ${mapping.salesforceField}: ${value}`);
    } else {
      console.warn(`No form data or picklist value found for field ${formFieldId || 'predefined'}`);
    }
  });

  if (!hasValidMapping) {
    const missingFields = actualFieldMappings
  ...
  // Step 4: Update matched records or create new one
  let result;

  if (matchedRecords.length > 0) {
    // Update existing records logic
    ...
    for (let i = 0; i < allRecordIds.length; i += MAX_BATCH_SIZE) {
      ...
      const batchRequests = chunk.map(recordId => ({
        method: 'PATCH',
      ...
      try {
        ...
        let batchResult = await batchResponse.json();

        if (!batchResponse.ok || batchResult.hasErrors) {
          if (batchResponse.status === 401 && !tokenRefreshed) {
            newAccessToken = await fetchNewAccessToken(userId);
            tokenRefreshed = true;
    ...
    // Upload files for updated records if file upload fields exist
    if (fileUploadFields.length > 0 && successfulIds.length > 0) {
      console.log('Starting file upload for updated records...');
      const recordIdForUpload = successfulIds[0]; // or loop through all
      const uploadResult = await uploadFilesToSalesforce(
        recordIdForUpload,
        fileUploadFields,
        formData,
        salesforceBaseUrl,
        tokenRefreshed ? newAccessToken : token
      );
    ...
    try {
      ...
      // Upload files if file upload fields exist
      if (fileUploadFields.length > 0 && responseData.id) {
        console.log('Starting file upload for new record...');
        const uploadResult = await uploadFilesToSalesforce(
          responseData.id,
          fileUploadFields,
          formData,
        ...
        if (!uploadResult.success) {
          console.warn('Record created successfully but file upload had errors');
        }
      }
...
const processLoopNode = async (node, previousResults, formData, salesforceBaseUrl, token) => {
  const { config, nodeId } = node;
  if (!config) {
    throw new Error(`Loop node ${nodeId} is missing loop configuration`);
  }

  const sourceCollection = previousResults[config.loopCollection];
  if (!sourceCollection || sourceCollection.error) {
    throw new Error(`Source collection ${config.loopCollection} is invalid or contains an error: ${JSON.stringify(sourceCollection)}`);
  }
  ...
  // Apply iteration order (ASC or DESC)
  let processedItems = [...itemsToProcess];
  if (config.loopIterationOrder === 'DESC') {
    processedItems = processedItems.reverse();
    console.log(`Processing loop in descending order. Original: ${itemsToProcess.length} items, Reversed: ${processedItems.length} items`);
  }

  const loopVariables = {
    currentItem: null,
    currentIndex: config.loopVariables?.currentIndex ? 0 : null,
    counter: config.loopVariables?.counter ? 1 : null,
    indexBase: parseInt(config.loopVariables?.indexBase || '0')
  };
  ...
  for (let i = 0; i < processedItems.length; i++) {
    ...
    loopVariables.currentItem = processedItems[i];

    // Handle index calculation based on iteration order
    if (loopVariables.currentIndex !== null) {
      if (config.loopIterationOrder === 'DESC') {
        // For descending order, calculate index from the end
        loopVariables.currentIndex = (processedItems.length - 1 - i) + loopVariables.indexBase;
      } else {
    ...
    const loopContext = {
      ...formData,
      [config.currentItemVariableName]: loopVariables.currentItem,
      ...(loopVariables.currentIndex !== null && {
        [`${config.currentItemVariableName}_index`]: loopVariables.currentIndex
      }),
      ...(loopVariables.counter !== null && {
        [`${config.currentItemVariableName}_counter`]: loopVariables.counter
      })
    };
  ...
  return {
    ...
    iterationOrder: config.loopIterationOrder || 'ASC',
    status: 'processed'
  };
};
...
const processFormatter = (config, formData) => {
  const { formatType, operation, inputField, inputField2, options, useCustomInput, customValue, outputVariable } = config;
  const result = {
    nodeId: config.nodeId || `formatter_${Date.now()}`,
    status: 'processed',
    operation,
    inputField,
  ...
  console.log('CONFIG IN FORMATTER:: ', config);
  console.log('RESULT IN FORMATTER:: ', result);
...
const runFlow = async (nodes, formData, instanceUrl, token, userId, formVersionId, submissionId) => {
  ...
  console.log('nodes in run flow:: ', JSON.stringify(nodes, null, 2));
      ...
      if (!skipUntilNextCondition) {
        ...
        switch (nodeType) {
          ...
          case 'Loop':
            ...
            const sourceCollection = results[node.config.loopCollection];
            if (!sourceCollection) {
              throw new Error(`Source collection ${node.config.loopCollection} not found in previous results`);
            }
          ...
          case 'Formatter':
            ...
            nodeResult = processFormatter(node.config, currentContext);
            console.log('nodeconfig:: ', node);
...
export const handler = async (event) => {
  try {
    ...
    console.log("nodes in handler::", JSON.stringify(nodes, null, 2));

    const parsedNodes = nodes.map(node => {

      // Handle fieldMappings that might contain content document config
      let fieldMappings = node.fieldMappings;
      ...
      // Filter out content document config if it exists
      let contentDocumentMappings = [];
      if (Array.isArray(fieldMappings)) {
        // Extract content document configs separately
        contentDocumentMappings = fieldMappings.filter(mapping => mapping?.storeAsContentDocument);

        // Keep only standard mappings
        fieldMappings = fieldMappings.filter(mapping =>
          mapping && mapping.formFieldId !== undefined && !mapping.storeAsContentDocument
        );
      }

      // Parse config to include loopIterationOrder
      let config = node.Config__c ? JSON.parse(node.Config__c) : node.config;
      if (config && config.loopConfig) {
        config = {
          ...config,
          loopIterationOrder: config.loopIterationOrder || 'ASC'
        };
      }
      ...
      return {
        ...
        contentDocumentMappings: contentDocumentMappings,
        config: config,
      };
    });
...
function simplifyLoopInput(inputData) {
  ...
  const { config, previousResults } = inputData.nodeConfig;

  return {
    sourceCollection: config?.loopCollection,
    itemsToProcess: previousResults?.[config?.loopCollection]?.ids?.length || 0,
    iterationOrder: config?.loopIterationOrder || 'ASC'
  };
}
...
function simplifyLoopOutput(output) {
  ...
  return {
    ...
    iterationOrder: output.iterationOrder || 'ASC',
    status: output.status
  };
}
...
function simplifyFormatterInput(inputData) {
  ...
  const { formatType, operation, inputField, options } = inputData.config;
