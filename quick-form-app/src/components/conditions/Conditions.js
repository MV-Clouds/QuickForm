...
const Conditions = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const metadataResponse = await fetch(
          process.env.REACT_APP_FETCH_METADATA_URL,
          {
            method: "POST",
            headers: {
        ...
        if (!formVersion)
          throw new Error(`Form version ${formVersionId} not found`);
        ...
        const parsedConditions = Array.isArray(formVersion.Conditions)
          ? formVersion.Conditions.flat().map((c) => ({
              ...
              logicExpression:
                c.logic === "Custom" ? c.logicExpression || "" : "",
            }))
          : [];
        ...
        const uniquePageNumbers = [
          ...new Set(formVersion.Fields.map((field) => field.Page_Number__c)),
        ].sort((a, b) => a - b);
        console.log("Unique Page Numbers:", uniquePageNumbers);
        const derivedPages = uniquePageNumbers.map((pageNum, index) => ({
          Id: `page_${pageNum}`,
          Name: `Page ${pageNum}`,
          Fields: formVersion.Fields.filter(
            (f) => f.Page_Number__c === pageNum
          ).map((f) => f.Unique_Key__c),
        }));
        ...
        const pageEdges = Object.values(conditionGroups)
          .map((group, idx) => {
            ...
            const offset =
              (idx -
                (Object.values(conditionGroups).filter(
                  (g) =>
                    g.source === group.source &&
                    g.thenAction === group.thenAction
                ).length -
                  1) /
                  2) *
              50;
            return {
              ...
              target: group.target,
              label:
                group.thenAction === "loop"
                  ? `Loop (${group.conditions.length} condition${
                  ...
                  : group.conditions
                      .map(
                        (c) =>
                          `${c.thenAction} ${
                            pages.find((p) => p.Id === c.targetPage[0])?.Name ||
                            "Unknown"
                          }`
                      )
                      ?.join(", "),
              type: "smoothstep",
              animated:
                group.thenAction === "skip to" || group.thenAction === "loop",
              style: {
                stroke:
                  group.thenAction === "skip to"
                    ? "#1a73e8"
                    : group.thenAction === "hide"
                    ? "#ff4d4f"
                    : "#00cc00", // Green for loop
                strokeWidth: 2,
                zIndex: 10 + idx,
              },
        ...
        const defaultEdges = derivedPages.slice(0, -1).map((page, index) => {
          ...
          const targetNode = pageNodes.find(
            (n) => n.id === derivedPages[index + 1].Id
          );
          return {
  ...
  const checkForContradiction = (newCondition, existingConditions) => {
    ...
    for (const condition of existingConditions) {
      ...
      if (newCondition.type === "dependent") {
        ...
        isSameIf =
          newCondition.conditions?.length ===
      ...
      if (newCondition.type === "show_hide" && isSameIf) {
        if (
          oppositeActions[conditionData.thenAction] ===
            newCondition.thenAction &&
        ...
        JSON.stringify(conditionData.dependentValues) !==
          JSON.stringify(newCondition.dependentValues)
      ) {
        ...
        if (newCondition.thenAction === "loop") {
          if (
            ...
            conditionData.sourcePage === newCondition.sourcePage
          ) {
        ...
        if (
          ...
          (newCondition.targetPage === pages[0]?.Id ||
            newCondition.targetPage === pages[pages.length - 1]?.Id)
        ) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (expectOperand) {
        if (/^\d+$/.test(token)) {
          ...
          return `Invalid token at position ${
            i + 1
          }: Expected number or '(', got ${token}.`;
        }
      } else {
  ...
  const editCondition = (condition) => {
    ...
    setNewCondition({
      ...
      conditions:
        conditionData.type === "dependent"
          ...
          : conditionData.conditions || [
              { ifField: "", operator: "", value: "" },
            ],
      logic:
        conditionData.type !== "dependent"
          ? conditionData.logic || "AND"
          : "AND",
      logicExpression:
        conditionData.logic === "Custom"
          ? conditionData.logicExpression || ""
          : "",
      thenAction:
        conditionData.thenAction ||
        (conditionData.type === "enable_require_mask"
          ? "require"
      ...
      ifField:
        conditionData.type === "dependent" ? conditionData.ifField || "" : "",
      value:
        conditionData.type === "dependent" ? conditionData.value || "" : "",
  ...
  const saveCondition = async (conditionId = null) => {
    ...
    try {
      ...
      if (newCondition.logic === "Custom") {
        const validationError = validateCustomLogic(
          newCondition.logicExpression,
          newCondition.conditions.length
        );
        if (validationError) throw new Error(validationError);
      }
      const contradictionError = checkForContradiction(
        newCondition,
      ...
      if (newCondition.type === "show_hide") {
        ...
        if (
          ...
          throw new Error(
            "Missing required fields for skip/hide page condition"
          );
        }
        const derivedPages = [
          ...new Set(fields.map((field) => field.Page_Number__c)),
        ]
          .sort((a, b) => a - b)
          .map((pageNum) => ({
            Id: `page_${pageNum}`,
        ...
        if (
          newCondition.thenAction === "hide" &&
          newCondition.targetPage.some(
            (page) =>
              page === derivedPages[0]?.Id ||
              page === derivedPages[derivedPages.length - 1]?.Id
          )
        ) {
        ...
        if (
          ...
          throw new Error(
            "Missing required fields for enable/require/mask condition"
          );
        }
        if (
          newCondition.thenAction === "set mask" &&
          !newCondition.maskPattern
        ) {
        ...
        conditionData = {
          ...
          Condition_Data__c: JSON.stringify({
            ...
            thenFields: newCondition.thenFields,
            ...(newCondition.thenAction === "set mask"
              ? { maskPattern: newCondition.maskPattern }
              : {}),
            ...(newCondition.thenAction === "unmask"
              ? { maskPattern: null }
              : {}),
          }),
        };
      ...
      if (newCondition.type === "skip_hide_page") {
        ...
        const targetPages = Array.isArray(newCondition.targetPage)
          ? newCondition.targetPage
          : [newCondition.targetPage].filter(Boolean);
          ...
          if (newCondition.thenAction === "loop") {
            ...
            if (existingEdge) {
              const updatedConditions = conditionId
                ? existingEdge.data.conditions.map((c) =>
                    c.Id === conditionId ? data.condition : c
                  )
              ...
              newEdges = newEdges.map((e) =>
                e.id === edgeId
                  ? {
                      ...
                      label: `Loop (${updatedConditions.length} condition${
                        updatedConditions.length > 1 ? "s" : ""
                      })`,
                      animated: true,
                      style: {
                        stroke: "#00cc00",
              ...
              const newEdge = {
                ...
                sourceX: nodes.find((n) => n.id === newCondition.sourcePage)
                  ?.position.x,
                sourceY:
                  nodes.find((n) => n.id === newCondition.sourcePage)?.position
                    .y + 50,
                targetX: nodes.find((n) => n.id === newCondition.sourcePage)
                  ?.position.x,
                targetY:
                  nodes.find((n) => n.id === newCondition.sourcePage)?.position
                    .y - 50,
                ...
                ...newEdges.filter(
                  (e) =>
                    !(
                      ...
                      e.id.startsWith(`${newCondition.sourcePage}-loop`)
                    )
                ),
                newEdge,
              ];
            ...
            targetPages.forEach((targetPage) => {
              ...
              if (existingEdge) {
                    ...
                    ? {
                        ...
                        label: updatedConditions
                          .map(
                            (c) =>
                              `${c.thenAction} ${
                                pages.find((p) => p.Id === c.targetPage[0])
                                  ?.Name || "Unknown"
                              }`
                          )
                          .join(", "),
                        animated: updatedConditions.some(
                          (c) => c.thenAction === "skip to"
                        ),
                        style: {
                          stroke: updatedConditions.some(
                            (c) => c.thenAction === "skip to"
                          )
                            ? "#1a73e8"
                            : "#ff4d4f",
                          strokeWidth: 2,
                          zIndex:
                            newEdges.findIndex((e) => e.id === edgeId) + 10,
                        },
                      }
                    : e
                );
                ...
                const newEdge = {
                  ...
                  source: newCondition.sourcePage,
                  target: targetPage,
                  label: `${data.condition.thenAction} ${
                    pages.find((p) => p.Id === targetPage)?.Name || "Unknown"
                  }`,
                  type: "smoothstep",
                  animated: data.condition.thenAction === "skip to",
                  style: {
                    stroke:
                      data.condition.thenAction === "skip to"
                        ? "#1a73e8"
                        : "#ff4d4f",
                    strokeWidth: 2,
                    zIndex: 10,
                  ...
                  sourceX: nodes.find((n) => n.id === newCondition.sourcePage)
                    ?.position.x,
                  sourceY:
                    nodes.find((n) => n.id === newCondition.sourcePage)
                      ?.position.y + 50,
                  targetX: nodes.find((n) => n.id === targetPage)?.position.x,
                  targetY:
                    nodes.find((n) => n.id === targetPage)?.position.y - 50,
                };
                newEdges = [
                  ...newEdges.filter(
                    (e) =>
                      !(
                        e.source === newCondition.sourcePage &&
                        e.id.startsWith(
                          `${newCondition.sourcePage}-${newCondition.thenAction}-`
                        ) &&
      ...
      setNewCondition({
        ...
        logicExpression: "",
        thenAction:
          newCondition.type === "enable_require_mask"
            ? "require"
            : newCondition.type === "skip_hide_page"
            ? "skip to"
            : "show",
        thenFields: [],
        dependentField: "",
        ...
        const updatedEdges = prevEdges
          .map((edge) => {
            if (edge.data?.conditions.some((c) => c.Id === conditionId)) {
              const updatedConditions = edge.data.conditions.filter(
                (c) => c.Id !== conditionId
              );
              return {
                ...edge,
                data: { conditions: updatedConditions },
                label:
                  edge.data.conditions[0]?.thenAction === "loop"
                    ? `Loop (${updatedConditions.length} condition${
                        updatedConditions.length > 1 ? "s" : ""
                      })`
                    : updatedConditions
                        .map(
                        ...
                        ?.join(", "),
                animated: updatedConditions.some(
                  (c) => c.thenAction === "skip to" || c.thenAction === "loop"
                ),
                style: {
                  stroke: updatedConditions.some(
                    (c) => c.thenAction === "skip to"
                    ...
                    ? "#1a73e8"
                    : updatedConditions.some((c) => c.thenAction === "hide")
                    ? "#ff4d4f"
                    : "#00cc00",
                  strokeWidth: 2,
  ...
  const validIfFields = fields.filter(
    (f) =>
      ![
        ...
        "section",
        "header",
      ].includes(f.Field_Type__c)
  );

  const validControllingFields = fields.filter((f) =>
    ["checkbox", "radio", "dropdown"].includes(f.Field_Type__c)
  );
  const validDependentFields = fields.filter((f) =>
    ["dropdown", "multiselect"].includes(f.Field_Type__c)
  );
  ...
  const getDependentValues = (fieldId) => {
    ...
    try {
      const properties = JSON.parse(field.Properties__c || "{}");
      return ["dropdown", "multiselect"].includes(field.Field_Type__c)
        ? properties.options || []
        : [];
    } catch (err) {
    ...
    setNewCondition((prev) => {
      if (prev.type === "dependent") {
        return {
          ...
          [field]: value,
          ...(field === "ifField"
            ? { value: "", dependentField: "", dependentValues: [] }
            : {}),
      ...
      return {
        ...
        [field]: value,
        ...(field === "thenFields" ? { thenFields: value } : {}),
        ...(field === "thenAction" && !["set mask", "unmask"].includes(value)
          ? { maskPattern: "" }
          : {}),
  ...
  const removeDependentValue = (index) => {
    const updatedValues = newCondition.dependentValues.filter(
      (_, i) => i !== index
    );
    setNewCondition({ ...newCondition, dependentValues: updatedValues });
  };
  ...
  const edgeTypes = {
    customBezier: ({
      ...
      if (source === target) {
        ...
        const path = `
          M ${x} ${y}
          C ${x + radiusX} ${y}, ${x + radiusX} ${y - radiusY * 2}, ${x} ${
          y - radiusY * 2
        }
      ...
      const offset =
        ...
        ((edges.filter((e) => e.source === source && e.target === target)
          ...
          2;
      const midX =
        (sourceX || nodes.find((n) => n.id === source).position.x) + offset;
      const midY =
        (sourceY || nodes.find((n) => n.id === source).position.y + 50) +
        ((targetY || nodes.find((n) => n.id === target).position.y - 50) -
          (sourceY || nodes.find((n) => n.id === source).position.y + 50)) /
          2;
      const path = `M ${
        sourceX || nodes.find((n) => n.id === source).position.x
      } ${sourceY || nodes.find((n) => n.id === source).position.y + 50} 
                    C ${midX} ${
        (sourceY || nodes.find((n) => n.id === source).position.y + 100) +
        offset
      },
                      ${midX} ${
        (targetY || nodes.find((n) => n.id === target).position.y - 100) +
        offset
      },
                      ${
                        targetX || nodes.find((n) => n.id === target).position.x
                      } ${
        targetY || nodes.find((n) => n.id === target).position.y - 50
      }`;
      return (
          ...
          {isLoading ? (
            ...
            >
                  ...
                  <div className="mb-4">
                        ...
                        <div className="mb-3">
                          ...
                          >
                            {validIfFields.map((field) => (
                              <Option
                                key={field.Unique_Key__c}
                                value={field.Unique_Key__c}
                              >
                                {field.Name}
                              </Option>
                            ))}
                          ...
                          <>
                            <div className="mb-3">
                              ...
                              >
                                {getOperators(
                                  fields.find(
                                    (f) => f.Unique_Key__c === condition.ifField
                                  )?.Field_Type__c
                                ).map((op) => (
                              ...
                              !["is null", "is not null"].includes(
                                ...
                                <div className="mb-3">
                                  ...
                                  {["checkbox", "radio", "dropdown"].includes(
                                    fields.find(
                                      (f) =>
                                        f.Unique_Key__c === condition.ifField
                                    )?.Field_Type__c
                                  ) ? (
                                    <Select
                                      value={condition.value}
                                      onChange={(value) =>
                                        handleFieldChange("value", value, index)
                                      }
                                      placeholder="Select value"
                                      style={{ width: "100%" }}
                                    >
                                      {getFieldOptions(condition.ifField).map(
                                        (option) => (
                          ...
                          <div className="mt-2">
                            <label className="block text-sm font-medium text-gray-700">
                              ...
                              <span className="text-gray-500 text-xs">
                                (e.g., "1 OR (2 AND 3)" for{" "}
                                {newCondition.conditions.length} conditions)
                              </span>
                            </label>
                            <Input
                    ...
                    <div className="mb-3 flex items-center">
                      <div style={{ width: "120px", marginRight: "8px" }}>
                        ...
                        <Select
                          ...
                          onChange={(value) =>
                            handleFieldChange("thenAction", value)
                          }
                          style={{ width: "100%" }}
                        >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          {validIfFields
                            .filter(
                              (f) =>
                                !newCondition.conditions?.some(
                                  (c) => c.ifField === f.Unique_Key__c
                                )
                            )
                            .map((f) => (
                              <Option
                                key={f.Unique_Key__c}
                                value={f.Unique_Key__c}
                              >
                                {f.Name}
                              </Option>
                            ))}
                    ...
                    <div className="flex space-x-2 mt-4">
                      <Button
                        ...
                        disabled={
                          !newCondition.conditions?.every(
                            (c) => c.ifField && c.operator
                          ) || !newCondition.thenFields.length
                        }
                      >
                          ...
                          <div>
                              ...
                              <>
                                ...
                                {condition.conditions?.map((cond, index) => (
                                  <p key={index}>
                                    ...
                                    {fields.find(
                                      (f) => f.Unique_Key__c === cond.ifField
                                    )?.Name || "Unknown"}{" "}
                                    {cond.operator} {cond.value || "N/A"}
                                  </p>
                                ))}
                                <p>
                                  <strong>Logic:</strong>{" "}
                                  {condition.logicExpression || "N/A"}
                                </p>
                              </>
                            ) : (
                              <p>
                            ...
                            <p>
                              ...
                              {(Array.isArray(condition.thenFields)
                                ...
                                : fields.find(
                                    (f) =>
                                      f.Unique_Key__c === condition.thenFields
                                  )?.Name) || "None"}
                            </p>
                          </div>
              ...
              <TabPane tab="Dependent Picklist" key="dependent">
                ...
                >
                  <h2 className="text-xl font-semibold mb-3">
                    Add Dependent Picklist Condition
                  </h2>
                  <div className="mb-4">
                    <div className="mb-3">
                      <label className="block text-sm font-medium text-gray-700">
                      ...
                      <Select
                        ...
                        onChange={(value) =>
                          handleFieldChange("ifField", value)
                        }
                    ...
                    {newCondition.ifField && (
                      <div className="mb-3">
                        ...
                        <Select
                          ...
                          style={{ width: "100%" }}
                        >
                          {getFieldOptions(newCondition.ifField).map(
                            (option) => (
                              <Option key={option} value={option}>
                                {option}
                              </Option>
                    ...
                    <div className="mb-3">
                      ...
                      <Select
                        ...
                        onChange={(value) =>
                          handleFieldChange("dependentField", value)
                        }
                      ...
                      >
                        {validDependentFields
                          .filter(
                            (f) => f.Unique_Key__c !== newCondition.ifField
                          )
                          .map((field) => (
                            <Option
                              key={field.Unique_Key__c}
                        ...
                        {newCondition.dependentValues.map((value, index) => (
                          ...
                          >
                            <Select
                              value={value}
                              onChange={(val) =>
                                updateDependentValue(index, val)
                              }
                        ...
                        >
                          <div>
                            ...
                            <p>
                              ...
                              {condition.dependentValues?.join(", ") || "None"}]
                              for{" "}
                              {fields.find(
                                (f) =>
                                  f.Unique_Key__c === condition.dependentField
                              )?.Name || "Unknown"}
                            </p>
                          </div>
                          <div className="flex space-x-2">
              ...
              <TabPane
                tab="Enable/Require/Mask a Field"
                key="enable_require_mask"
              >
                <motion.div
                  variants={containerVariants}
                ...
                >
                  <h2 className="text-xl font-semibold mb-3">
                    Add Enable/Require/Mask Condition
                  </h2>
                  <div className="mb-4">
                    {newCondition.conditions?.map((condition, index) => (
                      <motion.div
                    ...
                    {newCondition.thenAction === "set mask" && (
                      <div className="mb-3">
                        <label className="block text-sm font-medium text-gray-700">
                          Mask Pattern{" "}
                          <span className="text-gray-500 text-xs">
                            (use @ to mask letters, # to mask numbers, * to mask
                            both)
                          </span>
                        </label>
                        <Input
                          ...
                          onChange={(e) =>
                            handleFieldChange("maskPattern", e.target.value)
                          }
                          placeholder="e.g., @@##**"
                          style={{ width: "100%" }}
                        />
                ...
                <motion.div variants={containerVariants} className="mt-4">
                  <h2 className="text-xl font-semibold mb-3">
                    Existing Enable/Require/Mask Conditions
                  </h2>
                  <AnimatePresence>
                    {conditions
                      ...
                      .map((condition) => (
                        ...
                        >
                          <div>
                            ...
                            <p>
                              <strong>Then:</strong> {condition.thenAction}
                              {condition.thenAction === "set mask"
                                ? ` with pattern "${
                                    condition.maskPattern || "N/A"
                                  }"`
                                : ""}{" "}
                              {(Array.isArray(condition.thenFields)
                                ? condition.thenFields
                ...
                >
                  ...
                  >
                    ...
                    >
                      <p style={{ margin: 0, fontSize: "12px" }}>
                        <span style={{ color: "#1a73e8", marginRight: "5px" }}>
                          ---
                        </span>{" "}
                        Skip To
                      </p>
                      <p style={{ margin: 0, fontSize: "12px" }}>
                        <span style={{ color: "#ff4d4f", marginRight: "5px" }}>
                          ---
                        </span>{" "}
                        Hide
                      </p>
                  ...
                  <Modal
                    title={`Conditions for ${
                      pages.find((p) => p.Id === selectedNode)?.Name || "Page"
                    }`}
                    ...
                    >
                      ...
                      <Tabs
                        ...
                        onChange={(key) => {
                          handleFieldChange(
                            "thenAction",
                            key === "loop" ? "loop" : "skip to"
                          );
                          handleFieldChange(
                            "targetPage",
                            key === "loop" ? [selectedNode] : []
                          );
                          handleFieldChange(
                            "conditions",
                            key === "loop"
                                ...
                                >
                                  {validIfFields
                                    .filter((f) => {
                                      ...
                                      const pageNum = page
                                        ? parseInt(page.Id.replace("page_", ""))
                                        : null;
                                      return (
                                        pageNum &&
                                        fields.find(
                          ...
                          <div className="mb-3 flex items-center">
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <Select
                                ...
                                onChange={(value) =>
                                  handleFieldChange("targetPage", value)
                                }
                        ...
                        <TabPane tab="Loop" key="loop">
                          <div className="mb-3">
                            ...
                            <Select
                              ...
                              onChange={(value) =>
                                handleFieldChange("loopType", value)
                              }
                              style={{ width: "100%" }}
                            >
                          ...
                          {newCondition.loopType === "static" ? (
                            <div className="mb-3">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  handleFieldChange("loopValue", e.target.value)
                                }
                            ...
                            <div className="mb-3">
                              ...
                              <Select
                                ...
                                onChange={(value) =>
                                  handleFieldChange("loopField", value)
                                }
                          ...
                          <div className="mb-3">
                            ...
                            >
                              <Option value={selectedNode}>
                                {pages.find((p) => p.Id === selectedNode)
                                  ?.Name || "Current Page"}
                              </Option>
                            </Select>
                          </div>
                          <div className="flex space-x-2 mt-4">
                            <Button
                              ...
                              disabled={
                                !newCondition.sourcePage ||
                                (newCondition.thenAction === "loop" &&
                                  !newCondition.loopField &&
                                  !newCondition.loopValue)
                              }
                            >
                              <FaSave style={{ marginRight: "4px" }} />{" "}
                              {editingConditionId
