...
const CreateFormWizard = ({ onClose }) => {
  ...
  const filteredFields = useMemo(() => {
    return (
      fieldsData[currentObject]?.filter(
  ...
  const fetchMetadata = async () => {
    ...
    try {
      ...
      const token =
        accessToken || (await fetchAccessToken(userId, instanceUrl));
  ...
  const fetchFieldsForObject = async (objectName) => {
    if (fieldsData[objectName]) {
      // Check if fields for this object are already fetched
  ...
  const moveAllObjects = useCallback(
    (direction) => {
      if (direction === "right") {
        // Check if moving all to selected
    ...
    [filteredMetadata, selectedObjects]
  ); // Dependencies for useCallback
  ...
  const toggleFieldSelection = useCallback(
    (fieldName) => {
      ...
      if (field.referenceTo && field.referenceTo.length > 0) {
        // Check if field has a reference
        const referencedObject = field.referenceTo[0]; // Get the referenced object
        if (!selectedObjects.includes(referencedObject)) {
          // Check if referenced object is selected
          setError(
      ...
      if (currentSelected.includes(fieldName)) {
        ...
        if (field.required) {
          ...
          setError(
            `Cannot remove ${field.name} from selected fields because it is required.`
          ); // Set error if required field
          return; // Exit function
        }
        setSelectedFields({
          ...selectedFields,
    ...
    [fieldsData, currentObject, selectedObjects, selectedFields]
  ); // Dependencies for useCallback

  // Move all fields between available and selected lists
  const moveAllFields = useCallback(
    (direction) => {
      ...
      if (direction === "right") {
        ...
        const availableFields = filteredFields
          .filter(
            (field) =>
              !(selectedFields[currentObject] || []).includes(field.name)
          ) // Get all unselected fields
        ...
        if (fieldsWithReferences.length > 0) {
          // Check if there are fields with references
          setError(
            `Cannot select ${fieldsWithReferences
              .map((f) => f.name)
              .join(", ")} because referenced objects are not selected.` // Set error if referenced objects are not selected
          );
          return; // Exit function
        }
        ...
        setSelectedFields({
          ...selectedFields,
          [currentObject]: [
            ...(selectedFields[currentObject] || []),
            ...availableFields,
          ], // Add all fields to selected list
        });
      } else {
    ...
    [currentObject, fieldsData, filteredFields, selectedFields, selectedObjects]
  ); // Dependencies for useCallback
    ...
    try {
      ...
      results.forEach(({ obj, fields }) => {
        ...
        const requiredFields = fields
          ...
          .map((field) => field.name); // Get required fields
    ...
    const objectInfo = selectedObjects.map((obj) => {
      ...
      return {
        ...
        fields: (selectedFields[obj] || []).map((fieldName) => {
          const field =
            fieldsData[obj]?.find((f) => f.name === fieldName) || {};
          return {
            name: fieldName,
              ...
              <div className="object-container">
                <div className="title-group">
                  ...
                  <div className="wizard-refresh" onClick={fetchMetadata}>
                    ...
                    >
                      <path
                        d="M3.44599 5.51184C3.15162 6.61034 3.18961 7.77145 3.55515 8.84835C3.92069 9.92525 4.59735 10.8696 5.49958 11.5619C6.40181 12.2542 7.48908 12.6635 8.6239 12.7379C9.75871 12.8123 10.8901 12.5485 11.875 11.9798M14.554 8.48784C14.8484 7.38935 14.8104 6.22824 14.4448 5.15134C14.0793 4.07444 13.4026 3.13011 12.5004 2.43779C11.5982 1.74546 10.5109 1.33622 9.37608 1.26183C8.24127 1.18744 7.10988 1.45123 6.12499 2.01984"
                        stroke="#5F6165"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                      />
                      <path
                        d="M0.75 7.5L3.25 5L5.75 7.5M12.25 6.5L14.75 9L17.25 6.5"
                        stroke="#5F6165"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                ...
                <div className="wizard-dual-list-row">
                  {" "}
                  {/* Object selection layout */}
                  <div className="wizard-list-col">
                    {" "}
                    ...
                    <div className="wizard-list-title-row">
                      ...
                      >
                        ...
                        >
                          <path
                            d="M6.61084 1L13.2231 7.62308C13.2747 7.67142 13.3157 7.72981 13.3438 7.79464C13.3718 7.85947 13.3863 7.92936 13.3863 8C13.3863 8.07064 13.3718 8.14054 13.3438 8.20536C13.3157 8.27019 13.2747 8.32858 13.2231 8.37692L6.61084 15"
                            stroke="#5F6165"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                          />
                          <path
                            d="M1 1L7.62308 7.62308C7.72174 7.72373 7.777 7.85906 7.777 8C7.777 8.14094 7.72174 8.27627 7.62308 8.37692L1 15"
                            stroke="#5F6165"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                    ...
                    <div className="wizard-listbox">
                      ...
                      >
                        <AnimatePresence>
                          {filteredMetadata
                            .filter(
                              (obj) => !selectedObjects.includes(obj.name)
                            )
                            .map((obj) => (
                              <motion.div
                                key={obj.name}
                              ...
                              >
                                <div className="flex-grow flex items-center">
                                  <div>
                                    ...
                                    <div className="wizard-listbox-entry-api">
                                      {obj.name}
                                    </div>
                                  </div>
                                </div>
                                <button
                                  onClick={() =>
                                    toggleObjectSelection(obj.name)
                                  }
                                  className="wizard-list-add"
                                >
                  ...
                  <div className="wizard-list-col">
                    ...
                    <div className="wizard-list-title-row">
                      ...
                      >
                        ...
                        >
                          <path
                            d="M8.38916 15L1.77685 8.37692C1.72534 8.32858 1.68429 8.27019 1.65622 8.20536C1.62816 8.14053 1.61368 8.07064 1.61368 8C1.61368 7.92936 1.62816 7.85946 1.65622 7.79464C1.68429 7.72981 1.72534 7.67142 1.77685 7.62308L8.38916 0.999999"
                            stroke="#5F6165"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                          />
                          <path
                            d="M14 15L7.37692 8.37692C7.27826 8.27627 7.223 8.14094 7.223 8C7.223 7.85906 7.27826 7.72373 7.37692 7.62308L14 0.999999"
                            stroke="#5F6165"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                      ...
                      >
                        <AnimatePresence>
                          {selectedObjects.length > 0 ? (
                            <motion.div layout>
                              {" "}
                              {/* This enables layout animation for child positioning */}
                              {selectedObjects.map((objName) => {
                                const obj = metadata.find(
                                  (o) => o.name === objName
                                );
                                if (!obj) return null;
                                return (
            ...
            <div>
              <div className="object-container">
                <div className="title-group">
                  ...
                  <div
                    className="wizard-refresh"
                    onClick={refreshFieldsForCurrentObject}
                  >
                    <svg
                      width="18"
                      height="14"
                ...
                <div className="wizard-form-group">
                  {" "}
                  {/* Object dropdown section */}
                  <label className="object-small">Select Object:</label>
                  <Select
                    id="object-select"
                    value={currentObject}
                                  ...
                                  <div className="flex-grow flex items-center">
                                    <div>
                                      <div className="wizard-listbox-entry-label">
                                        {field.label}
                                        {field.required && (
                                          <span className="wizard-listbox-entry-required">
                                            *
                                          </span>
                                        )}
                                      </div>
                    ...
                    <div className="wizard-listbox selected">
                      ...
                      >
                        {(selectedFields[currentObject] || []).length > 0 ? (
                          <AnimatePresence>
                            {(selectedFields[currentObject] || []).map(
                              (fieldName) => {
