...
const FOLDER_ICON = (
  ...
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M21.5923 1.85536C20.8579 0.876107 19.7052 0.299805 18.4812 0.299805H4.63889C2.49112 0.299805 0.75 2.04091 0.75 4.18869V41.8163C0.75 43.964 2.49112 45.7052 4.63889 45.7052H52.8611C55.0089 45.7052 56.75 43.964 56.75 41.8163V13.2698C56.75 11.1221 55.0089 9.38089 52.8611 9.38089H29.1809C27.9568 9.38089 26.8043 8.80457 26.0698 7.82533L21.5923 1.85536Z"
      fill="url(#paint0_linear_1568_12948)"
    />
    <defs>
      <linearGradient
        id="paint0_linear_1568_12948"
        x1="28.75"
        y1="0.299805"
        x2="28.75"
        y2="45.7052"
  ...
  const allForms = useMemo(
    () =>
      recentForms.map((f) => ({
        id: f.id || f.Id,
        name:
          f.FormVersions.filter((version) => version.Stage__c === "Publish")[0]
            ?.Name ||
          f.FormVersions[0]?.Name ||
          "Form",
        folder: f.Folder__c,
      })),
    [recentForms]
    ...
    <div className="mt-6 px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        ...
        {paginatedFolders.map((folder, idx) => (
          <div
            key={folder.name}
            className="flex  items-center bg-white rounded-xl shadow p-6 hover:shadow-lg transition-all border border-yellow-100"
          >
            <div className="mb-3 ">{FOLDER_ICON}</div>
            <div className="ml-2">
              <div className="font-bold text-lg  mb-1 truncate max-w-[150px]">
                {folder.name}
              </div>
