...
const allApps = [
  {
    ...
    name: "Twilio",
    description:
      "Twilio lets you send SMS and make calls through powerful APIs. Easily add communication features to any app.",
    icon: <SiTwilio size={32} color="#F22F46" />,
  },
  {
    id: "square",
    name: "Square",
    description:
      "Square offers tools for payments, POS, and business management. Accept payments anywhere and grow your business with ease.",
    icon: <SiSquare size={32} color="#000" />,
  },
  {
    id: "razorpay",
    name: "Razorpay",
    description:
      "Razorpay is a full-stack payment solution for businesses in India.",
    icon: <SiRazorpay size={32} color="#0B295E" />,
  },
  {
    id: "stripe",
    name: "Stripe",
    description:
      "Stripe is a global payment platform for online businesses. It enables easy, secure payment processing and financial services integration.",
    icon: <SiStripe size={32} color="#1D6D9E" />,
  },
  {
    id: "paypal",
    name: "PayPal",
    description:
      "PayPal is a global leader in online payment solutions with millions of account holders worldwide.",
    icon: (
      <img
  ...
  },
  {
    ...
    name: "AWS",
    description:
      "Amazon Web Services (AWS) is a comprehensive cloud computing platform that offers a wide range of services for computing, storage, databases, analytics, machine learning, and more.",
    icon: (
      <img
  ...
  },
  {
    ...
    name: "Google Sheet",
    description:
      "Google Sheets is a free online spreadsheet editor that allows users to create and edit spreadsheets online and collaborate with others in real-time.",
    icon: <SiGooglesheets size={32} color="green" />,
  },
  {
    id: "gmail",
    name: "Gmail",
    description:
      "Gmail is a free email service provided by Google that allows users to send and receive emails with a generous storage capacity and advanced security features.",
    icon: (
      <img
...
const initialConnectedApps = [
  {
    id: "razorpay",
    name: "Razorpay",
    description: "Connected to your account. Payment gateway for India.",
    icon: <SiRazorpay size={32} color="#0B295E" />,
  },
];
      ...
      if (href.startsWith(GMAIL_REDIRECT_URI)) {
        ...
        try {
          ...
          const res = await fetch(
            "https://40npk4h6n3.execute-api.us-east-1.amazonaws.com/auth",
            {
              method: "POST",
...
const Integrations = ({ token }) => {
  ...
  // Whether PayPal is considered connected is now handled inside PayPalIntegrationPanel when opened.
    ...
    try {
      const res = await fetch(
        `https://40npk4h6n3.execute-api.us-east-1.amazonaws.com/auth?instanceUrl=${encodeURIComponent(
          instanceUrl
        )}&sfToken=${encodeURIComponent(token)}`,
        { method: "GET" }
      );
      if (!res.ok) {
      ...
      const googleApps = mapped
        .filter(
          (integration) =>
            (integration.tokenType === "gmail" ||
              integration.tokenType === "google-sheet") &&
        ...
        .map((integration) => {
          ...
          if (app) {
            return {
              ...app,
              description: `Connected to your ${app.name} account.`,
            };
          }
          return null;
        })
  ...
  }, [token, google_access_token, fetchIntegrations]);
    ...
    if (app.id === "aws") {
      ...
      try {
        const response = await fetch(
          "https://pm8ylpazok.execute-api.us-east-1.amazonaws.com/auth",
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          ...
          setConnectedApps((prev) => [
            ...prev,
            { ...app, description: "Connected to your AWS account." },
          ]);
          setSelectedApp(null);
          setAwsAccessKey("");
          setbucketname("");
      ...
      try {
        ...
        const response = await fetch(
          "https://naf0rbkhc6.execute-api.us-east-1.amazonaws.com/twilioAuth",
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (response.ok) {
          ...
          setConnectedApps((prev) => [
            ...prev,
            { ...app, description: `Connected as ${twilioConnectionName}.` },
          ]);
          setSelectedApp(null);
          setTwilioSid("");
          setTwilioAuthToken("");
      ...
      openGoogleOAuthPopup(
        ...
        (accessToken) => {
          ...
          setConnectedApps((prev) => [
            ...prev,
            { ...app, description: "Connected to your Gmail account." },
          ]);
          setSelectedApp(null);
        },
        (error) => {
      ...
      openGoogleOAuthPopup(
        ...
        (accessToken) => {
          ...
          setConnectedApps((prev) => [
            ...prev,
            { ...app, description: "Connected to your Google Sheets account." },
          ]);
          setSelectedApp(null);
        },
        (error) => {
      ...
      setConnectedApps((prev) => [
        ...prev,
        { ...app, description: `Connected to your ${app.name} account.` },
      ]);
      setSelectedApp(null);
    }
  };
  ...
  return (
    <div>
      ...
      >
        <div
          className="px-10 py-8 rounded-b-xl shadow-lg relative"
          ...
          {!selectedApp && (
            <>
              <div className="flex border-b mb-6 mt-8">
                ...
                <button
                  className={`ml-4 px-4 py-2 -mb-px border-b-2 font-medium transition-colors  ${
                    activeTab === "connected"
                      ? ""
                      : "text-gray-500 hover:text-blue-600"
                  }`}
              ...
              {activeTab === "all" && (
                <div>
                  <div className="flex items-center mb-6 justify-between">
                    ...
                    />
                    <button className="ml-4 px-5 py-2 border border-2  rounded-lg text-gray-600 flex items-end justify-right gap-2">
          ...
          {selectedApp && (
            <div>
              ...
              >
                ...
                <div
                  className="flex flex-row items-center justify-center gap-6 mb-6 py-20 w-[100%] "
                  style={{
                    background:
                      "url('images/Framehd.png') center center / cover no-repeat, white",
                  }}
                >
                  {/* App Icon */}
                ...
                {selectedApp.id === "twilio" && !twilioConnected && (
                  <div className="w-full">
                    ...
                    <div className="mb-6">
                      <label
                        className="block text-gray-700 text-sm font-bold mb-2"
                        htmlFor="twilio-connection-name"
                      >
                        Connection Name
                      </label>
                      <input
                ...
                {selectedApp.id === "aws" && awsConnected && awsUser && (
                  <div className="text-center text-green-700 text-sm mb-2">
                    Connected as:{" "}
                    {awsUser.username || awsUser.userId || "AWS User"}
                  </div>
                )}
                  ...
                  <div className="flex justify-between gap-4 w-full mt-2">
                    ...
                    {selectedApp.id === "aws" && awsConnected ? (
                      ...
                      <button
                        ...
                        disabled={connecting}
                      >
                        <div className="mr-2 p-0.5 rounded-lg bg-white flex items-center">
                          <div
                            className={`${
                              selectedApp.id === "aws" ? "w-5 h-5" : ""
                            }`}
                          >
                            {selectedApp.icon}
                          </div>
                        </div>
                        {connecting && selectedApp.id !== "aws"
                          ? "Connecting..."
                          : "Connect"}
                      </button>
                    )}
              ...
              {(selectedApp?.id === "google" ||
                selectedApp?.id === "gmail" ||
                selectedApp?.id === "google-sheet") &&
                googleUsers.length > 0 && (
                  <div className="flex justify-center">
                    <div className="w-[42%]">
                      <h3 className="text-lg font-semibold mb-2">
                        Connected Google Users
                      </h3>
                      <motion.div
                        layout
                        initial={false}
