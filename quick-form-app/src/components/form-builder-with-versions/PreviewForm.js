...
import { evaluateFormula } from '../form-publish/FormulaEvaluator';
import PaymentFieldRenderer from "../form-publish/payment/PaymentFieldRenderer";
...
function PreviewForm({ formVersion, formFields, formConditions = [], prefills = [] }) {
  ...
  React.useEffect(() => {
    if (formFields && formFields.length > 0) {
      ...
      formFields.forEach((field) => {
        ...
        // Add to mapping
        if (properties.id && fieldId) {
          localIdMapping[properties.id] = fieldId;

          // Handle subfields (including nested subfields)
          if (properties.subFields && typeof properties.subFields === 'object') {
            Object.entries(properties.subFields).forEach(([subName, subField]) => {
              // Add null check for subField
              if (!subField) return;

              // For section subfields (leftField/rightField)
              if (field.Field_Type__c === 'section') {
                // Check if subField has an id before using it
                if (subField.id) {
                  localIdMapping[subField.id] = `${fieldId}_${subName}`;
                  // Handle nested subfields (like fullname subfields)
                  if (subField.subFields && typeof subField.subFields === 'object') {
                    Object.entries(subField.subFields).forEach(([nestedName, nestedField]) => {
                      // Add null check for nestedField
                      if (!nestedField) return;
                      if (nestedField.value !== undefined) { // Check if it's a subfield config
                        localIdMapping[`${subField.id}_${nestedName}`] = `${fieldId}_${subName}_${nestedName}`;
                      }
                    });
                  }
                }
              ...
              // For other field types with subfields (like address)
                ...
                localIdMapping[`${properties.id}_${subName}`] = `${fieldId}_${subName}`;
              }
            });
          }
        }
        ...
        if (fieldType === 'phone' && properties.subFields?.countryCode?.enabled) {
          initialValues[`${fieldId}_countryCode`] =
          ...
          initialValues[fieldId] = properties.defaultValue || [];
        } else if (fieldType === 'datetime' || fieldType === 'date') {
          initialValues[fieldId] = properties.defaultValue || null;
        ...
        initialSignatures[fieldId] = null;
        initialFilePreviews[fieldId] = null;
        initialRatings[fieldId] = null;
        initialSelectedOptions[fieldId] = fieldType === 'dropdown' && properties.allowMultipleSelections ? [] : '';
  ...
  const handleChange = (fieldId, value, isFile = false) => {
    ...
    setErrors((prev) => {
      ...
      // Also clear error for parent field if this is a subfield
  ...
  const calculateFormCalculation = (field) => {
    ...
    (properties.fieldReferences || []).forEach(refId => {
      ...
      const referencedField = formFields.find(f => f.Unique_Key__c === refId);
      if (referencedField) {
  ...
  const renderField = (field) => {
    ...
    const properties = JSON.parse(field.Properties__c || '{}');
    const state = getFieldUiState(properties.id || (field && field.Id) || field?.id, formValues);
    const isHidden = state.hidden || properties.isHidden;
    if (isHidden) return null;
    const fieldId = (field && field.Id) || properties.id;
    const fieldType = field.Field_Type__c;
    ...
    // Get alignment class based on labelAlignment
    const alignmentClass = alignmentStyles[labelAlignment] || alignmentStyles.top;
    ...
    switch (fieldType) {
              ...
              {(properties.options || ['Option 1', 'Option 2']).map((option, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-center">
                    ...
                    <label
                      htmlFor={`${fieldId}-${idx}`}
                      className={`ml-2 block text-sm ${isDisabledCheck && !isChecked ? 'text-gray-400' : 'text-gray-700'
                        }`}
                    >
                      {option}
                    </label>
              ...
              <div className="flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs text-gray-500">
                  {properties.allowedFileTypes
                    ? `Supported: ${properties.allowedFileTypes}`
                    : 'Any file type'}
                  {properties.maxFileSize && ` • Max size: ${properties.maxFileSize}MB`}
                </p>
              </div>
      ...
      case 'imageuploader':
        ...
        const previewImage =
          ...
          "/images/quickform-only-logo.png";
        ...
        return (
          <div className="mb-4">
            <div
              ...
              style={{
                width: properties.imageWidth ? `${properties.imageWidth}px` : "200px",
                height: properties.imageHeight ? `${properties.imageHeight}px` : "150px",
              }}
            >
      ...
      case 'section':
        ...
        // Simple section rendering without recursive field lookups
        const leftFieldConfig = properties.subFields?.leftField;
        const rightFieldConfig = properties.subFields?.rightField;
        ...
        return (
          <div className="mb-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              {leftFieldConfig && (
                <div>
                  <label className={`block text-sm font-medium mb-1 text-gray-700 ${labelAlignmentClass}`}>
                    {leftFieldConfig.label || 'Left Field'}
                    {leftFieldConfig.isRequired && <span className="text-red-500 ml-1">*</span>}
                  </label>
                  <input
                    type="text"
                    value={formValues[`${fieldId}_left`] || ''}
                    onChange={(e) => handleChange(`${fieldId}_left`, e.target.value)}
                    placeholder={leftFieldConfig.placeholder || ''}
                    className="w-full p-2 border rounded-md border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                    disabled={leftFieldConfig.isDisabled}
                  />
                </div>
              )}
              ...
              {rightFieldConfig && (
                <div>
                  <label className="block text-sm font-medium mb-1 text-gray-700">
                    {rightFieldConfig.label || 'Right Field'}
                    {rightFieldConfig.isRequired && <span className="text-red-500 ml-1">*</span>}
                  </label>
                  <input
                    type="text"
                    value={formValues[`${fieldId}_right`] || ''}
                    onChange={(e) => handleChange(`${fieldId}_right`, e.target.value)}
                    placeholder={rightFieldConfig.placeholder || ''}
                    className="w-full p-2 border rounded-md border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                    disabled={rightFieldConfig.isDisabled}
                  />
                </div>
              )}
            </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white rounded-lg inset-shadow-2xs">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">{formVersion?.Name || 'Form Preview'}</h1>
      <form className="space-y-6" aria-label="Public Form">
        <div className="page">
          {pages[currentPage]?.map((field) => (
            field && <div key={field.Unique_Key__c}>{renderField(field)}</div>
          ))}
        </div>
