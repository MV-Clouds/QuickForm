...
function MainFormBuilder({ showMapping, showThankYou, showNotification }) {
  ...
  const fetchFormData = async (userId, instanceUrl) => {
    try {
      ...
      originalFieldsRef.current = JSON.stringify(reconstructedFields);
    } catch (error) {
  ...
  // Set PayPal user context when userId or formId changes
  useEffect(() => {
    const userId = sessionStorage.getItem("userId");
    if (userId && formId) {
      console.log("🔍 MainFormBuilder: Setting PayPal user context", {
        userId,
        formId,
      });
  ...
  // Temporarily disable this useEffect to prevent infinite loops
  // The issue is that prepareFormData() creates new objects every time, causing infinite re-renders
  // We'll handle hasChanges differently to avoid this problem

  useEffect(() => {
    console.log(
      "🔍 MainFormBuilder: useEffect triggered for hasChanges check",
      {
        currentFormVersionExists: !!currentFormVersion,
    ...
    if (currentFormVersion) {
      ...
      //
      console.log("🔍 MainFormBuilder: Field changes check", {
        originalFieldsLength: originalFields.length,
  ...
  }, [currentFormVersion, fields]); // Add proper dependencies
  //
  ...
  const saveFormToSalesforce = async () => {
    ...
    try {
      ...
      console.log(formVersion, " 😂 ", formFields);
