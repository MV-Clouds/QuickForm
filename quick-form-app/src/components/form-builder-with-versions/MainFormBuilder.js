...
function MainFormBuilder({
 ...
 const [footerConfigs, setFooterConfigs] = useState({});
  ...
  const handlePreview = () => {
    setPreviewStep(1); // fade out sidebar
    setTimeout(() => setPreviewStep(2), 400); // show preview
    setTimeout(() => setPreviewStep(3), 500); // hide mainmenubar
    setTimeout(() => {
      const { formVersion, formFields } = prepareFormData();
      setPreviewFormData({ formVersion, formFields });
      setShowPreview(true);
    }, 1200); // show preview form
  };
  ...
  return (
    <div className="flex h-screen">
      <div className={previewStep < 3 ? "fade-in" : "fade-out"}>
        <MainMenuBar
  ...
  setFooterConfigs={setFooterConfigs}
                ...
                {!showPreview && (
                  <div className="w-1/4 pl-2 slide-in-right">
