...
const ImageUploader = ({ defaultImage, onImageUpload, style }) => {
  const [image, setImage] = useState(defaultImage);
  const [isHovered, setIsHovered] = useState(false);
  const fileInputRef = React.useRef(null);
  ...
  return (
    <div 
      className="relative bg-gray-100 rounded-sm overflow-hidden flex items-center justify-start"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      onClick={triggerFileInput}
      style={style}
    >
      ...
      {image && (
        <img
          src={image}
          alt="Upload preview"
          className={`h-full w-full object-cover transition-opacity duration-300 ${isHovered ? 'opacity-50' : 'opacity-100'}`}
          style={{
            maxWidth: '100%',
            objectFit: 'contain'
          }}
