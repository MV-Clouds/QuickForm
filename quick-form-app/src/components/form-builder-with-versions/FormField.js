  ...
  return (
    <div className="relative">
      ...
      <table className="w-full text-center border-collapse border border-gray-300 mt-6">
        <thead>
          <tr>
            <th className="p-2 border-t border-l border-r-0 border-transparent invisible">
              .
            </th>
            {columnValues.map((colLabel, colIdx) => (
              <th
                key={colIdx}
                className="border border-gray-300 bg-blue-100/50 p-2 text-center"
              >
                {editingColumn === colIdx ? (
                  ...
                  <span
                    onClick={() => setEditingColumn(colIdx)}
                ...
                >
                  ...
                  {inputType === "checkbox" && (
                    <input
                      ...
                      value={colLabel}
                      checked={(selectedValues[rowIdx] || []).includes(
                        colLabel
                      )}
        ...
        <div className="flex items-center gap-2">
          ...
          {showHelpText && helpText && (
            <span className="relative inline-block">
              ...
              <span
                className="absolute left-1/2 z-20 -translate-x-1/2 mt-2 w-48 bg-gray-800 text-white text-xs rounded p-2 shadow-lg opacity-0 pointer-events-none transition-opacity duration-200
...
function FormField({
  ...
  const {
    ...
    isRichText = false,
    longTextMaxChars,
    numberValueLimits = { enabled: false, min: "", max: "" },
    checkboxRelatedValues = {},
    radioRelatedValues = {},
    priceLimits = { enabled: false, min: "", max: "" },
    currencyType = "USD",
  ...
  const [plainTextValue, setPlainTextValue] = useState(
    field.value ? stripHtml(field.value) : ""
  );
  ...
  const [selectedOptions, setSelectedOptions] = useState(
    field.value || (allowMultipleSelections ? [] : "")
  );
  ...
  const handleOptionChange = (index, newValue) => {
    ...
    const relatedValuesKey =
      type === "checkbox" ? "checkboxRelatedValues" : "radioRelatedValues";
    const relatedValues =
      type === "checkbox" ? checkboxRelatedValues : radioRelatedValues;
    ...
    if (onUpdateField) {
      onUpdateField(id, {
        options: newOptions,
        [relatedValuesKey]: newRelatedValues,
      });
    }
  };
  ...
  const shouldDisableDate = (date) => {
    ...
    if (type === "date" && dateRange.start && date < new Date(dateRange.start))
      return true;
    if (type === "date" && dateRange.end && date > new Date(dateRange.end))
      return true;
    if (
      type === "datetime" &&
      datetimeRange.start &&
      date < new Date(datetimeRange.start)
    )
      return true;
    if (
      type === "datetime" &&
      datetimeRange.end &&
      date > new Date(datetimeRange.end)
    )
      return true;
  ...
  const renderCell = (date) => {
    ...
    return (
      <div
        style={isExplicitlyDisabled ? { textDecoration: "line-through" } : {}}
      >
        {date.getDate()}
      </div>
    );
      ...
      return (
        <SelectionWrapper>
          <FieldWrapper
            ...
            labelContent={
              <label
                className={`text-gray-700 ${selectedTheme?.textColor || ""}`}
              >
                {label || "Date"}
                {isHidden && (
          ...
          >
            ...
            >
              <DatePicker
                format={rsuiteToHtmlFormat(dateFormat).replace(
                  /-/g,
                  dateSeparator
                )}
                defaultValue={
                  defaultDate ? formatDateForRsuite(defaultDate) : null
                }
                disabled={isDisabled}
                disabledDate={shouldDisableDate}
                renderCell={renderCell}
                placeholder={placeholder.main || "Select a date"}
                className={`w-full ${selectedTheme?.inputText || ""} ${
                  selectedTheme?.inputBg || ""
                }`}
                ...
                onChange={(value) => {
                  if (value) {
                    onUpdateField(id, {
                      defaultDate: value.toISOString().split("T")[0],
                    });
                  }
                }}
                onOpen={() => {
      ...
      return (
        <SelectionWrapper>
          <FieldWrapper
            ...
            labelContent={
              <label
                className={`${selectedTheme?.textColor || "text-gray-700"}`}
              >
                {label || "Date and Time"}
                {isHidden && (
                  <FaEyeSlash className="text-gray-400" title="Hidden Field" />
          ...
          >
            ...
            >
              <DatePicker
                ...
                defaultValue={
                  defaultTime
                    ? formatDateForRsuite(`1970-01-01T${defaultTime}`)
                    : null
                }
                disabled={isDisabled}
                ...
                <div
                  className={`text-2xl font-bold ${
                    alignment === "left"
                      ...
                      ? "text-right"
                      : "text-center"
                  } w-full ${selectedTheme?.textColor || "text-gray-700"}`}
                >
                  {heading || "Form"}
                  {isHidden && (
                    <FaEyeSlash
      ...
      return (
        <SelectionWrapper>
          ...
          >
            ...
            {shortTextMaxChars && (
              <p className="text-sm text-gray-500 mt-1">
                Max characters: {shortTextMaxChars}
              </p>
            )}
          </FieldWrapper>
        </SelectionWrapper>
        ...
        <SelectionWrapper>
          <FieldWrapper
            labelContent={
              <label
                className={` ${
                  selectedTheme?.textColor || "text-gray-700color"
                }`}
              >
                {label || "Long Text"}
                {isHidden && (
              ...
              <div>
                ...
                <div className="flex justify-between text-xs text-gray-500 mt-1">
                  <span>
                    Words:{" "}
                    {plainTextValue.trim()
                      ? plainTextValue.trim().split(/\s+/).length
                      : 0}
                  </span>
                  <span>
                    Characters: {plainTextValue.length}
                    {longTextMaxChars ? ` / ${longTextMaxChars}` : ""}
                  </span>
                </div>
              </div>
            )}
        ...
        <SelectionWrapper>
          ...
          >
            <input
              ...
              value={numberValue}
              min={
                numberValueLimits.enabled ? numberValueLimits.min : undefined
              }
              max={
                numberValueLimits.enabled ? numberValueLimits.max : undefined
              }
            ...
            {numberValueLimits.enabled && (
              <p className="text-sm text-gray-500 mt-1">
                Range: {numberValueLimits.min || "No min"} -{" "}
                {numberValueLimits.max || "No max"}
              </p>
            )}
          </FieldWrapper>
        </SelectionWrapper>
              ...
              {subFields.countryCode?.enabled ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-1/3">
                    <PhoneInput
                      country={
                        subFields.countryCode?.value
                          ? subFields.countryCode.value.toLowerCase()
                          : "us"
                      }
                      value=""
                      onChange={(phone, countryData) => {
                        ...
                        handleSubFieldChange("countryCode", {
                      ...
                      disabled={isDisabled}
                      placeholder={
                        subFields.countryCode?.placeholder ||
                        "Select country code"
                      }
                      enableSearch
                  ...
                  <div className="w-2/3">
                    <input
                      ...
                      placeholder={
                        subFields.phoneNumber?.placeholder ||
                        "Enter phone number"
                      }
                      disabled={isDisabled}
                ...
                <InputMask
                  ...
                  onChange={(e) =>
                    handleSubFieldChange("phoneNumber", {
                      ...
                      phoneMask:
                        subFields.phoneNumber?.phoneMask || "(999) 999-9999",
                    })
                  }
                  className={`p-2 border rounded w-full text-sm ${
                    selectedTheme?.inputText || ""
                  ...
                  placeholder={
                    subFields.phoneNumber?.placeholder || "Enter phone number"
                  }
                  disabled={isDisabled}
              ...
              {localOptions.map((opt, idx) => (
                <div key={opt} className="flex items-center gap-2">
                  ...
                  <div
                    ...
                    className={`p-1 rounded flex-grow ${
                      selectedTheme?.inputText || ""
                    } ${selectedTheme?.inputBg || ""}`}
                    ...
                    ? selectedOptions.length > 0
                      ? selectedOptions
                          .map(
                            (opt) =>
                              localOptions.find(
                                (o) => (dropdownRelatedValues[o] || o) === opt
                              ) || opt
                          )
                          .join(", ")
                ...
                <div className="absolute w-full mt-1 bg-white border rounded shadow-lg z-10 max-h-40 overflow-y-auto">
                  {localOptions.map((opt, idx) => (
                    <div
                      ...
                      onClick={() =>
                        toggleOption(dropdownRelatedValues[opt] || opt)
                      }
                    >
                      {opt}
                    </div>
          ...
          >
            ...
            >
              ...
              {imagePreview ? (
                ...
                <div
                  className={`h-32 text-sm p-2 border rounded bg-gray-100 flex items-center justify-center text-gray-700 hover:bg-gray-200 cursor-pointer ${
      ...
      return (
        <SelectionWrapper>
          ...
          >
            ...
            {priceLimits.enabled && (
              <p className="text-sm text-gray-500 mt-1">
                Range: {priceLimits.min || "No min"} -{" "}
                {priceLimits.max || "No max"}
              </p>
            )}
          </FieldWrapper>
        </SelectionWrapper>
        ...
        <SelectionWrapper>
          ...
          >
            <div className="flex gap-3">
              {subFields.salutation?.enabled && (
                <div className="w-1/5">
                  <select
                    ...
                    onChange={(e) =>
                      handleSubFieldChange("salutation", {
                        value: e.target.value,
                      })
                    }
                    className={`w-full p-2 border rounded ${
                  ...
                  >
                    <option value="" disabled>
                      {subFields.salutation?.placeholder || "Select Salutation"}
                    </option>
                    {(
                      subFields.salutation?.options || [
                        "Mr.",
                        "Mrs.",
                        "Ms.",
                        "Dr.",
              ...
              <div
                className={subFields.salutation?.enabled ? "w-2/5" : "w-1/2"}
              >
                <input
                  type="text"
                  className={`w-full p-2 border rounded ${
                  ...
                  onChange={(e) =>
                    handleSubFieldChange("firstName", { value: e.target.value })
                  }
                  readOnly={isDisabled}
                  pattern={field?.validation?.pattern}
                  title={field?.validation?.description}
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    handleSubFieldChange("lastName", { value: e.target.value })
                  }
                  readOnly={isDisabled}
                />
              </div>
              ...
              {subFields.street?.visible && (
                <div className="w-full">
                  ...
                  <input
                    ...
                    placeholder={
                      subFields.street?.placeholder || "Street Address"
                    }
                    value={subFields.street?.value || ""}
                    onChange={(e) =>
                      handleSubFieldChange("street", { value: e.target.value })
                    }
                    readOnly={isDisabled}
                  />
                </div>
                  ...
                  {subFields.country?.visible && (
                    <div className="w-1/2">
                      ...
                      <input
                        ...
                        placeholder={
                          subFields.country?.placeholder || "Country"
                        }
                        value={subFields.country?.value || ""}
                        onChange={(e) =>
                          handleSubFieldChange("country", {
                            value: e.target.value,
                          })
                        }
                        readOnly={isDisabled}
                  ...
                  {subFields.postal?.visible && (
                    <div className="w-1/2">
                      ...
                      <input
                        ...
                        placeholder={
                          subFields.postal?.placeholder || "Postal Code"
                        }
                        value={subFields.postal?.value || ""}
                        onChange={(e) =>
                          handleSubFieldChange("postal", {
                            value: e.target.value,
                          })
                        }
                        readOnly={isDisabled}
        ...
        <SelectionWrapper>
          ...
          >
            <input
              ...
              placeholder={
                placeholder.main || "Calculation result will appear here"
              }
              value={formula}
              readOnly
              pattern={field?.validation?.pattern}
          ...
          >
            <DynamicScaleRating
              ...
              onUpdateColumns={(newColumns) =>
                onUpdateField(id, { columns: newColumns })
              }
            />
          </FieldWrapper>
        </SelectionWrapper>
    ...
    case "section":
      return (
        <SelectionWrapper isSection>
          <div className="flex gap-2">
            <div
              className={`w-1/2 min-h-[100px] rounded ${
                subFields.leftField
                  ? "border-gray-300"
